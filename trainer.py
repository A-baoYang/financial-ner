...
from utils import MODEL_CLASSES, compute_metrics, get_slot_labels, set_seed
...
class Trainer(object):
    ...
    def train(self):
        ...
        logger.info("   Num train examples = %d", len(self.train_dataset))
        ...
        train_iterator = trange(
            int(self.args.num_train_epochs), desc="Epoch"
        )  # 进度条，如分成10组
        ...
        for _ in train_iterator:
            ...
            logger.info("   Step = %d", num_step)
            logger.info("   Loss = %d", tr_loss)
            ...
            for k, v in res.items():
                if k not in res_dict.keys():
                    ...
                    res_dict[k].append(v)
            if "train_loss" not in res_dict.keys():
    ...
    def evaluate(self, mode):
        ...
        logger.info("   Num eval examples = %d", len(dataset))
        ...
        # results.update({"true_pred_list": [(out_slot_label_list, slot_preds_list)]})
        results.update(total_result)

        logger.info("*****   Eval results  *****")
        for key in sorted(results.keys()):
            if key != "true_pred_list":
    ...
    def _convert_texts_to_tensors(
        ...
        for text in texts:
            ...
            if len(tokens) > self.args.max_seq_len - special_tokens_count:
                ...
                slot_label_mask = slot_label_mask[
                    : (self.args.max_seq_len - special_tokens_count)
                ]
        ...
        input_ids_batch = torch.tensor(input_ids_batch, dtype=torch.long).to(
            self.device
        )
        attention_mask_batch = torch.tensor(attention_mask_batch, dtype=torch.long).to(
            self.device
        )
        token_type_ids_batch = torch.tensor(token_type_ids_batch, dtype=torch.long).to(
            self.device
        )
        slot_label_mask_batch = torch.tensor(
            slot_label_mask_batch, dtype=torch.long
        ).to(self.device)
    ...
    def predict(self, texts, tokenizer):
        ...
        with open(
            os.path.join(self.args.pred_dir, self.args.pred_output_file),
            "a",
            encoding="utf-8",
        ) as f:
